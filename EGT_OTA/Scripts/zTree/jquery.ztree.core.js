...
(function($) {
	...
	handler = {
	    onSwitchNode: function(event, node) {
	        ...
	        //[2011-09-09]edit by laone 实现打开节点时关闭其同级节点
	        if (setting.closeSameLevel && !node.open) {//打开菜单时是否关闭同级的菜单
	            var siblingNodes = (node.getParentNode()) ? node.getParentNode()[setting.data.key.childs] : data.getRoot(setting)[setting.data.key.childs];
	            if (siblingNodes) {
	                for (var son = 0; son < siblingNodes.length; son++) {
	                    if (siblingNodes[son].open) view.switchNode(setting, siblingNodes[son]);
	                }
	            }
	        }
	        //edit end
	    ...
	    onClickNode: function(event, node) {
	        ...
	        //[2011-09-17] edit by laone
	        if (setting.titleClickSwitchChildren) {
	            handler.onSwitchNode(event, node);
	        }
	        //edit end
	        return true;
	    },
	...
	view = {
	    ...
	    appendNodes: function(setting, level, nodes, parentNode, initFlag, openFlag) {
	        ...
	        for (var i = 0, l = nodes.length; i < l; i++) {
	            ...
	            if (openFlag) {
	                ...
	                if (node.customStyle) {//[201-09-17]edit by laone
	                    html.push("<a id='", node.tId, consts.id.A, "' class='level", node.level, " ", node.customStyle.cssClass, "' treeNode", consts.id.A, " customNode='1' onclick=\"", (node.click || ''),
						"\" ", ((url != null && url.length > 0) ? "href='" + url + "'" : ""), " target='", view.makeNodeTarget(node), "' style='", fontStyle.join(''),
						"'");
	                    if (tools.apply(setting.view.showTitle, [setting.treeId, node], setting.view.showTitle)) { html.push("title='", node[titleKey].toString().replace(/'/g, "&#39;"), "'"); }
	                    html.push(">");
	                    data.getInnerBeforeA(setting, node, html);
	                    html.push("<span id='", node.tId, consts.id.SPAN,
						"'>", node[nameKey].toString().replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'), "</span>");
	                    ...
	                    html.push("<button type='button' id='", node.tId, consts.id.SWITCH,
						...
						"'>", node[nameKey].toString().toString().replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'), "</span>");
	                    data.getInnerAfterA(setting, node, html);
	                    html.push("</a>");
	                    data.getAfterA(setting, node, html);
	                    if (node.isParent && node.open) {
	    ...
	    makeUlHtml: function(setting, node, html, content) {
	        //[2011-09-17]edit by laone 设置子菜单的高度
	        var style = [];
	        style.push("style='display:", (node.open ? "block;" : "none;"));
	        if (node.customStyle) {
	            style.push("width:", node.customStyle.subWidth, "px;");
	            style.push("height:", node.customStyle.subHeight, "px;");
	        }
	        style.push("'");
	        //edit end
	        html.push("<ul id='", node.tId, consts.id.UL, "' class='level", node.level, " ", view.makeUlLineClass(setting, node), "' ", style.join(""), ">");
	        html.push(content);
	        html.push("</ul>");
	    },
	    ...
	    selectNode: function(setting, node, addFlag) {
	        ...
	        //[20-09-18] edit by laone
	        if (node.customStyle) {
	            $("#" + setting.treeId).find("a." + node.customStyle.current).removeClass(node.customStyle.current);
	            $("#" + node.tId + consts.id.A).addClass(node.customStyle.current);
	        } //edit end
	        else {
