...
(function($) {
	...
	_bindEvent = function(setting) {
	    ...
	    var c = consts.event;
	    o.unbind(c.RENAME);
	    o.bind(c.RENAME, function(event, treeId, treeNode, oldName) {
	        tools.apply(setting.callback.onRename, [event, treeId, treeNode, oldName]);
	    });
	...
	_zTreeTools = function(setting, zTreeTools) {
	    ...
	    zTreeTools.removeNode = function(node, applyBeforRemove) {
	        if (!node) return;
	        if (applyBeforRemove) {
	        ...
	        if (node) {
	            ...
	            if (newName !== node[nameKey]) {
	                if (tools.apply(setting.callback.beforeRename, [setting.treeId, node, newName], true) === false) {
	                    ...
	                    var oldName = node[nameKey];
	                    node[nameKey] = newName ? newName : inputObj.val();
	                    setting.treeObj.trigger(consts.event.RENAME, [setting.treeId, node, oldName]);
	                }
	            }
	            var aObj = $("#" + node.tId + consts.id.A);
