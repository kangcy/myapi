...
namespace EGT_OTA.Controllers.Api
{
    public class CommentController : BaseApiController
    {
        ...
        /// </summary>
        [HttpPost]
        [Route("Api/Comment/Edit_1_3")]
        public string Edit_1_3([FromBody] CommentApi api)
        {
            ApiResult result = new ApiResult();
            try
            {
                ...
                var ArticleNumber = Tools.SafeString(api.ArticleNumber);
                if (string.IsNullOrWhiteSpace(ArticleNumber))
                {
                    result.message = "文章信息异常";
                    return JsonConvert.SerializeObject(result);
                ...
                var summary = SqlFilter(UrlDecode(Tools.SafeString(api.Summary)), false, false);
                if (string.IsNullOrWhiteSpace(summary))
                {
                    result.message = "请填写评论内容";
                    return JsonConvert.SerializeObject(result);
                ...
                model.CreateUserNumber = user.Number;
                model.CreateIP = Tools.GetClientIP;
                model.ParentCommentNumber = Tools.SafeString(api.ParentCommentNumber);
                model.ParentUserNumber = Tools.SafeString(api.ParentUserNumber);

                model.Province = Tools.SafeString(api.Province);
                model.City = Tools.SafeString(api.City);
                model.District = Tools.SafeString(api.District);
                model.Street = Tools.SafeString(api.Street);
                model.DetailName = Tools.SafeString(api.DetailName);
                model.CityCode = Tools.SafeString(api.CityCode);
                model.Latitude = Tools.SafeDouble(api.Latitude);
                model.Longitude = Tools.SafeDouble(api.Longitude);
                model.ShowPosition = api.ShowPosition;
                model.ID = Tools.SafeInt(db.Add<Comment>(model));
                if (model.ID > 0)
                {
                    result.result = true;
            ...
            {
                LogHelper.ErrorLoger.Error("Api_Comment_Edit_1_3:" + ex.Message);
                result.message = ex.Message;
            }
            return JsonConvert.SerializeObject(result);
        }
